...
- **Warcode free-publication quote exception (scoped):** Under `games/the_warcode/rules/`, `games/the_warcode/setup/`, `games/the_warcode/factions/`, and `games/the_warcode/lore/` only, you may quote **verbatim** from the freely distributed rulebook and lorebook committed at [`raw/the_warcode/`](raw/the_warcode/). Every quote block: filename + **page** + section heading (optional stable IDs in `Rulebook_Quotes.md`). **OCR** flattened image pages (e.g. Protocol Cards) when native text is empty; cite `via OCR`. **Hierarchy:** `The-Warcode-Rulebook-V.0.8.9-F.pdf` is the current rules baseline and supersedes v0.8.7-F on the same topic; omission is not a patch. The Tactical Doctrine Field Edition lorebook supplies narrative context only and never overrides mechanics in the rulebook. **Binary exception:** these free PDFs and extraction/OCR sidecars under `raw/the_warcode/` may be tracked in git — see [`.gitignore`](.gitignore) negation. **Never** commit STL files. **`KB/` and `docs/` stay paraphrase** + `[[wikilink]]` to quote files. **Shipping naming (GW proper-noun ban):** In `games/the_warcode/**`, these proper nouns must **never** appear (any casing): **Kill Team**; **Warhammer**; **Warhammer 40,000** / **Warhammer 40K** combinations; **40,000**; **40K**; **40k**. Use obfuscated equivalents instead:
...
| [`raw/the_warcode/`](raw/the_warcode/) free publications | The Warcode v0.8.9-F rules baseline and Tactical Doctrine lore source (see Sec 10 Warcode exception) |
...
- v0.9.1 (2026-09-24): Sec 10 — Warcode v0.8.9-F is the current free rules baseline; scoped verbatim exception extended to the free Tactical Doctrine lorebook under `games/the_warcode/lore/**`; KB/docs remain paraphrase-only.
