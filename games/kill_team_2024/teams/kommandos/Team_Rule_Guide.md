<!--
FILE: games/kill_team_2024/teams/kommandos/Team_Rule_Guide.md
VERSION: v0.5.4 (2026-08-21)
OWNER: Russell Catt
AUTHOR_OF_NOTES: Cursor (Implementer, learn_to_play_event S1)
DOCUMENT_TYPE: Team Rule Guide (quoted + teaching)
GAME_SYSTEM: Kill Team — 2024 / 3e (KT24)
TEAM: Kommandos
SOURCES:
  - C:\Personal\Kill Team\kill_team_2024\Teams\eng_17-06_kill_team_team_rules_kommandos_online_rules-ova8v1kjds-ds3ouz4k04.pdf (read in place 2026-08-21)
-->

# Team Rule Guide — Kommandos

**Personal use only. Never for sale.** Verbatim blocks from `eng_17-06_kill_team_team_rules_kommandos_online_rules-ova8v1kjds-ds3ouz4k04.pdf`, read **2026-08-21**.

---

## Faction rule — Throat Slittas (quoted)

**Source:** Teams PDF p.4

> **THROAT SLITTAS**
>
> Each friendly KOMMANDO operative (excluding BOMB SQUIG) can perform the Charge action while it has a Conceal order.

**What it means:** You do not have to flip to Engage to charge. Stay Concealed for cover and Skulk About, then charge from Conceal when you are ready.

---

## Roster (quoted)

**Source:** Teams PDF p.4 / p.12

> - 1 KOMMANDO BOSS NOB operative with one of the following options:
>   - Slugga; big choppa
>   - Slugga; power klaw
> - 9 KOMMANDO operatives selected from the following list:
>   - BOMB SQUIG
>   - BOY
>   - BREACHA BOY
>   - BURNA BOY
>   - COMMS BOY
>   - DAKKA BOY
>   - GROT
>   - ROKKIT BOY
>   - SLASHA BOY
>   - SNIPA BOY
>
> Other than BOY operatives, your kill team can only include each operative on this list once.
>
> These operatives count as half a selection each, meaning you can select both of them and it’s treated as one selection in total.

**What it means (half selections):** **Bomb Squig** and **Grot** are the half-pair — take both and they cost **one** of your nine selections together. Boys can be taken more than once; specialists cannot.

**Archetypes:** Infiltration, Seek & Destroy.

---

## Strategy ploys (quoted names + key lines)

**Source:** Teams PDF pp.4–5

### Dakka! Dakka! Dakka!

> Friendly KOMMANDO operatives’ ranged weapons have the Punishing weapon rule.

### Sssshhhh!

> Each friendly KOMMANDO operative that’s not a valid target for enemy operatives, or has a Conceal order and is more than 6" from enemy operatives, can immediately perform a free Dash action. You cannot use this ploy during the first turning point.

### Waaagh!

> Friendly KOMMANDO operatives’ melee weapons have the Balanced weapon rule.

### Skulk About

> Whenever an enemy operative is shooting a friendly KOMMANDO operative that has a Conceal order, you can retain one of your defence dice as a normal success without rolling it (in addition to a cover save, if any).

---

## Firefight ploys (quoted)

**Source:** Teams PDF pp.5–6

### Just a Scratch

> Use this firefight ploy when an attack dice inflicts Normal Dmg on a friendly KOMMANDO operative (excluding BOMB SQUIG and GROT). Ignore that inflicted damage.

### Shake It Off

> Use this firefight ploy when a friendly KOMMANDO operative is activated, or when its APL stat is changed. Until the start of the next turning point, you can ignore any changes to its APL stat.

### Krump ’Em!

> Use this firefight ploy at the end of the Firefight phase. Select one friendly KOMMANDO operative. It can immediately perform a free Fight action.

### Kunnin’ But Brutal

> Use this firefight ploy when a friendly KOMMANDO operative that has a Conceal order is fighting during an activation in which it performed the Charge action, you’re resolving the first attack dice, and it’s a strike with a normal success. Treat that normal success as a critical success instead.

---

## Faction equipment (quoted summaries)

**Source:** Teams PDF pp.6–7

| Equipment | Cheat (see PDF for full text) |
|-----------|-------------------------------|
| **Choppas** | Friendly Kommandos (not Squig/Grot) gain a choppa melee profile if they lack a better one |
| **Collapsible Stocks** | Remove Range from shokka pistol / slugga |
| **Dynamite** | Once per battle Blast explosive (excl. Squig/Grot) |
| **Harpoon** | Once per TP ranged stun/lethal tool (excl. Squig/Grot) |

---

## Printable datacards

See [`cards/Card_Schema.md`](cards/Card_Schema.md).

## Related pages

- [`operatives/Operatives_Index.md`](operatives/Operatives_Index.md)
- [`Starter_Roster.md`](Starter_Roster.md)
- [`Quick_Reference_Play_Guide.md`](Quick_Reference_Play_Guide.md)
- [`Volkus_Playbook.md`](Volkus_Playbook.md)

## Change Log

- v0.5.4 (2026-08-21): Initial quoted guide (learn_to_play_event S1).

## Attribution

- Kill Team is Copyright Games Workshop Limited 2024. Personal use only.
