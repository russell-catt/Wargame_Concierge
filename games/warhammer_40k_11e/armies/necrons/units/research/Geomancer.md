# Geomancer

- **name:** Geomancer
- **slug:** Geomancer
- **wahapedia_url:** https://wahapedia.ru/wh40k11ed/factions/necrons/Geomancer
- **research_date:** 2026-08-19
- **role_tags:** Character, Cryptek, Support
- **priority:** inventory_candidate
- **completeness:** draft

## Composition
1 model per unit. Equipped with tremorglaive.

## Profiles
| M | T | SV | W | LD | OC |
|---|---|----|---|----|-----|
| 8" | 4 | 4+ | 4 | 6+ | 1 |

## Ranged weapons
| Weapon | Range | A | BS | S | AP | D | Keywords |
|--------|-------|---|----|---|----|---|----------|
| Tremorglaive — reverberating beam | 18" | 2 | 4+ | 8 | -2 | 2 | Melta 2 |
| Tremorglaive — shock wave pulse | 18" | D6+2 | N/A | 4 | 0 | 1 | Ignores Cover, Torrent |

## Melee weapons
| Weapon | Range | A | WS | S | AP | D |
|--------|-------|---|----|---|----|---|
| Tremorglaive | Melee | 2 | 4+ | 8 | -2 | 2 |

## Abilities
- **Support** (Core)
- **Reanimation Protocols** (Faction)
- **Tectonic Reverberations:** In your Movement phase, pin one visible enemy within 18" (−2 Move, −2 Charge until your next Movement phase)
- **Obelisk Node Control:** While on an objective you control, enemy Reserves cannot arrive within 12"
- **Vanguard Protocols:** If attached to Canoptek Macrocytes at Battle Formations, gains Scouts 8"

## Leader / attach options
**Support** — Canoptek Macrocytes, Immortals, Necron Warriors (MFM v1.2).

## Keywords
Infantry, Character, Cryptek, Geomancer, Faction: Necrons

## Points / cost notes
**75 pts** — MFM v1.2 (read 2026-08-19).

## List-building notes
Owned via Kill Team: Tomb World (not Hierotek). Primary Warlord for Conclave learning lists.

## Sources
- Wahapedia: https://wahapedia.ru/wh40k11ed/factions/necrons/Geomancer (retrieved 2026-08-19)
- MFM v1.2: C:\Personal\40K\rules\Warhammer 40,000_ Munitorum Field Manual.pdf
- Attestation: teaching paraphrase for private repo use



## At the table (plain language)
- **Reanimation:** End of your **Command phase** (`08.05`) — roll D3 wounds; heal first, then return models at 1W. Wiped units stay dead. See `Reanimation_Protocols.md`.
- **Objectives:** Each model has **OC** (`14.02`) — add up OC near a marker like counting who holds a flag.
- **Shooting:** Pick targets you can see (`06.01`). Cover makes shots worse (`13.08`) unless a rule says `[IGNORES COVER]`.

---
Verify vs current Munitorum / faction pack — patches happen | 2026-08-19
