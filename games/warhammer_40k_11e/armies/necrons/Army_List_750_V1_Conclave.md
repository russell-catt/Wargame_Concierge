<!--
FILE: games/warhammer_40k_11e/armies/necrons/Army_List_750_V1_Conclave.md
VERSION: v1.0 (2026-08-19)
OWNER: Russell Catt
AUTHOR_OF_NOTES: Cursor (cloud agent)

DOCUMENT_TYPE: Army List / Fully Statted Play Sheet
PROJECT_NAME: Wargame_Concierge
GAME_SYSTEM: Warhammer 40,000 - 11th Edition
FACTION: Necrons
DETACHMENT: Cryptek Conclave
REFERENCE_STATUS: Active - 750 V1. Points MFM v1.2 (read 2026-08-19). Stats Wahapedia 2026-08-19. confidence: draft.

SOURCES:
  - games/warhammer_40k_11e/armies/necrons/Starter_Forces_500_750_1000.md
  - games/warhammer_40k_11e/armies/necrons/Army_List_500_V1_Conclave.md
  - C:\Personal\40K\rules\Warhammer 40,000_ Munitorum Field Manual.pdf (v1.2)
  - https://wahapedia.ru/wh40k11ed/factions/necrons (2026-08-19)

PURPOSE:
  Fully statted Cryptek Conclave 750 V1 — no new purchases; Hierotek proxies OK.

PRIMARY_AUDIENCE:
  - Table play after 500 V1 list

UPDATE_TRIGGER:
  Update when sprue built, proxies replaced, or MFM changes.
-->

# Army list - 750 pts V1 - Cryptek Conclave (no purchases)

> **FORCE DISPOSITION — write on the list before you deploy**
> **Priority Assets** · MFM: `CRYPTEK CONCLAVE - 2DP - PRIORITY ASSETS`
> Matching Primaries: [`Cryptek_Conclave_Primary_Missions.md`](Cryptek_Conclave_Primary_Missions.md) · How matching works: [`../../setup/Chapter_Approved_Force_Dispositions.md`](../../setup/Chapter_Approved_Force_Dispositions.md)

| Field | Value |
|-------|-------|
| **Points** | **750 / 750** |
| **Purchases** | **$0** — Hierotek proxies OK |
| **Bench first** | 2nd Warriors, 2nd Scarabs, Immortals sprue |
| **Prior list** | [`Army_List_500_V1_Conclave.md`](Army_List_500_V1_Conclave.md) |

---

## Roster

| # | Unit / upgrade | Models | Pts |
|---|----------------|--------|-----|
| 1 | Cryptek Geomancer *(Warlord)* | 1 | 75 |
| 2 | Technomancer | 1 | 80 |
| 3 | Plasmancer *(Apprentek proxy)* | 1 | 55 |
| 4 | Royal Warden *(Despotek proxy)* | 1 | 50 |
| 5 | Necron Warriors | 10 | 80 |
| 6 | Necron Warriors *(2nd squad — build)* | 10 | 80 |
| 7 | Canoptek Tomb Crawlers | 2 | 50 |
| 8 | Canoptek Macrocytes | 5 | 70 |
| 9 | Canoptek Scarab Swarms | 6 | 80 |
| 10 | Immortals | 5 | 70 |
| 11 | **Gauntlet of Compression** | — | 20 |
| 12 | **Gravitic Bolas** | — | 15 |
| 13 | **Quantum Abacus** | — | 15 |
| | | | **750** |

**Attachments:** Warden + Geomancer on one Warrior squad; Technomancer or Plasmancer proxy on Immortals. Second Warriors, Macrocytes, Crawlers, Scarabs for board control.

---

## Unit stat blocks

Duplicate profiles match [`Army_List_500_V1_Conclave.md`](Army_List_500_V1_Conclave.md) for: Geomancer, Technomancer, Plasmancer proxy, Royal Warden proxy, Warriors, Immortals, Scarabs.

### Canoptek Tomb Crawlers (2) — 50 pts

| Model | M | T | SV | W | LD | OC |
|-------|---|---|----|---|----|----|
| Tomb Crawler | 5" | 4 | 3+ | 3 | 8+ | 1 |

| Weapon | Range | A | BS | S | AP | D | Keywords |
|--------|-------|---|----|---|----|---|----------|
| Twin gauss reaper | 12" | 2 | 4+ | 4 | -1 | 1 | `[LETHAL HITS]`, `[TWIN-LINKED]` |
| Transdimensional isolator | 12" | 2 | 4+ | 4 | -2 | 2 | — |
| Claws | Melee | 4 | 4+ | 6 | -1 | 1 | — |

**Owned loadout:** 1× twin gauss reapers, 1× transdimensional isolator.

---

### Canoptek Macrocytes (5) — 70 pts

| Model | M | T | SV | W | LD | OC |
|-------|---|---|----|---|----|----|
| Canoptek Macrocyte | 8" | 3 | 4+ | 1 | 8+ | 1 |

| Weapon | Range | A | BS | S | AP | D | Keywords |
|--------|-------|---|----|---|----|---|----------|
| Gauss scalpel | 18" | 1 | 4+ | 4 | -1 | 1 | `[LETHAL HITS]` |
| Tesla caster | 18" | 1 | 4+ | 5 | 0 | 1 | `[ASSAULT]`, `[SUSTAINED HITS 1]` |
| Atomiser beam | 12" | 1 | 4+ | 6 | -1 | 1 | — |
| Claws | Melee | 2 | 4+ | 4 | -1 | 1 | — |

**Owned loadout:** 1× accelerator mandible, 1× nanoscarab projector, 3× gauss scalpel/tesla (2 scalpel, 1 tesla). **Scouts 8".** Harassment Swarm: enemies within 3" suffer −1 Hit (non-Monster/Vehicle).

---

## Enhancements (750 pad)

| Enhancement | Pts | Bearer suggestion |
|-------------|-----|-------------------|
| Gauntlet of Compression | 20 | Geomancer or Technomancer — +6" range to unit guns |
| Gravitic Bolas | 15 | Either Cryptek — slow/charge brake |
| Quantum Abacus | 15 | Either Cryptek — CP when unit targeted by stratagem |

---

## Related pages

- [`Starter_Forces_500_750_1000.md`](Starter_Forces_500_750_1000.md)
- [`Army_List_1000_V2_Conclave.md`](Army_List_1000_V2_Conclave.md)

---

## Change Log
- v1.0 (2026-08-19): Initial fully statted 750 V1. MFM v1.2 re-cost (Crawlers 50, Macrocytes 70); enhancements rebalance to 750.

## Attribution
- Project: Wargame_Concierge | Maintainer: Russell Catt
- Teaching paraphrase. Warhammer 40,000 is Copyright Games Workshop Limited.
