<!--
FILE: games/warhammer_40k_11e/armies/necrons/Army_List_1000_V2_Conclave.md
VERSION: v1.0 (2026-08-19)
OWNER: Russell Catt
AUTHOR_OF_NOTES: Cursor (cloud agent)

DOCUMENT_TYPE: Army List / Fully Statted Play Sheet
PROJECT_NAME: Wargame_Concierge
GAME_SYSTEM: Warhammer 40,000 - 11th Edition
FACTION: Necrons
DETACHMENT: Cryptek Conclave
REFERENCE_STATUS: Active - 1000 V2 (purchases: Plasmancer, Szeras, Lychguard). MFM v1.2 read 2026-08-19. Stats Wahapedia 2026-08-19. confidence: draft.

SOURCES:
  - games/warhammer_40k_11e/armies/necrons/Starter_Forces_500_750_1000.md
  - C:\Personal\40K\rules\Warhammer 40,000_ Munitorum Field Manual.pdf (v1.2)
  - https://wahapedia.ru/wh40k11ed/factions/necrons (2026-08-19)

PURPOSE:
  Fully statted finished Cryptek Conclave 1,000 pt list (V2 purchases).

UPDATE_TRIGGER:
  Update when kits arrive or MFM changes.
-->

# Army list - 1,000 pts V2 - Cryptek Conclave (purchases)

| Field | Value |
|-------|-------|
| **Points** | **1,000 / 1,000** |
| **Buy** | Plasmancer + Illuminor Szeras + Lychguard (5) |
| **Prior lists** | [`Army_List_750_V1_Conclave.md`](Army_List_750_V1_Conclave.md) |

---

## Roster

| # | Unit / upgrade | Models | Pts | Source |
|---|----------------|--------|-----|--------|
| 1 | Cryptek Geomancer *(Warlord)* | 1 | 75 | OWNED |
| 2 | Technomancer | 1 | 80 | OWNED — Hierotek |
| 3 | **Plasmancer** | 1 | 55 | **BUY** |
| 4 | Royal Warden *(Despotek proxy)* | 1 | 50 | OWNED proxy |
| 5 | Necron Warriors | 20 | 190 | OWNED (merge) |
| 6 | Immortals | 5 | 70 | OWNED |
| 7 | Canoptek Scarab Swarms | 6 | 80 | OWNED |
| 8 | Canoptek Tomb Crawlers | 2 | 50 | OWNED |
| 9 | Canoptek Macrocytes | 5 | 70 | OWNED |
| 10 | **Illuminor Szeras** | 1 | 175 | **BUY** |
| 11 | **Lychguard** | 5 | 80 | **BUY** |
| 12 | **Gauntlet of Compression** | — | 20 | Enhancement |
| 13 | **Atomic Disintegrators** | — | 10 | Enhancement |
| 14 | **Gravitic Bolas** | — | 15 | Enhancement |
| | | | **1,000** | |

**Attachments:** Warden + Geomancer on 20 Warriors; Technomancer or Plasmancer on Immortals; Szeras behind Warrior phalanx; Lychguard screen characters.

---

## New / purchase unit stat blocks

### Illuminor Szeras — 175 pts

| Model | M | T | SV | W | LD | OC | INV |
|-------|---|---|----|---|----|----|---|-----|
| Illuminor Szeras | 8" | 8 | 2+ | 9 | 6+ | 3 | 4+ |

| Weapon | Range | A | BS/WS | S | AP | D | Keywords |
|--------|-------|-----|-------|---|----|---|----------|
| Eldritch lance (ranged) | 36" | 3 | 3+ | 9 | -3 | 3 | — |
| Eldritch lance (melee) | Melee | 4 | 3+ | 9 | -3 | 3 | — |
| Impaling legs | Melee | 4 | 3+ | 6 | -1 | 1 | Extra Attacks |

**Abilities:** FNP 4+; Lone Operative near friendlies; Mechanical Augmentation aura (+1 AP for friendly Battleline attacks within 3", −1 AP vs them).

---

### Lychguard (5) — 80 pts

| Model | M | T | SV | W | LD | OC |
|-------|---|---|----|---|----|----|
| Lychguard | 5" | 5 | 3+ | 2 | 7+ | 1 |

| Weapon | Range | A | WS | S | AP | D | Keywords |
|--------|-------|---|----|---|----|---|----------|
| Hyperphase sword | Melee | 3 | 3+ | 6 | -2 | 1 | — |
| Warscythe | Melee | 2 | 3+ | 8 | -3 | 2 | `[DEVASTATING WOUNDS]` |

**Default:** warscythe. Shield variant: hyperphase sword + dispersion shield (4+ invulnerable). Guardian Protocols with Noble leader.

---

## Other units

See [`Army_List_750_V1_Conclave.md`](Army_List_750_V1_Conclave.md) and [`Army_List_500_V1_Conclave.md`](Army_List_500_V1_Conclave.md) for Geomancer, Technomancer, Plasmancer, Warden, Warriors (×20 same profile as ×10), Immortals, Scarabs, Tomb Crawlers, Macrocytes.

---

## Related pages

- [`Starter_Forces_500_750_1000.md`](Starter_Forces_500_750_1000.md)
- [`Cryptek_Conclave.md`](Cryptek_Conclave.md)

---

## Change Log
- v1.0 (2026-08-19): Initial fully statted 1000 V2. Rebalanced vs stale Crawler/Macrocyte points; Macrocytes included; three enhancements to 1,000.

## Attribution
- Project: Wargame_Concierge | Maintainer: Russell Catt
- Teaching paraphrase. Warhammer 40,000 is Copyright Games Workshop Limited.
