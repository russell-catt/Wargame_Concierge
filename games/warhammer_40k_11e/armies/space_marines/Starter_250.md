<!--
FILE: games/warhammer_40k_11e/armies/space_marines/Starter_250.md
VERSION: v0.6.0 (2026-08-21)
OWNER: Russell Catt
AUTHOR_OF_NOTES: Cursor (Implementer — ownership update pass 2026-08-21)

DOCUMENT_TYPE: Learning List / Starter Guide
PROJECT_NAME: Wargame_Concierge
GAME_SYSTEM: Warhammer 40,000 - 11th Edition
FACTION: Space Marines
DETACHMENT: Gladius Task Force
REFERENCE_STATUS: Active for owned Blood Ravens paths; legacy Primaris paths retained. Points from MFM Marines v1.2 (read 2026-08-16 / re-checked via page render 2026-08-21)

SOURCES:
  - C:\Personal\40K\rules\Warhammer 40,000_ Munitorum Field Manual_Marines.pdf (v1.2, printed 13 Aug 2026)
  - C:\Personal\40K\rules\eng_22-07_warhammer_40,000_faction_pack_space_marines.pdf (v1.1)
  - games/warhammer_40k_11e/armies/space_marines/Owned_Models_Inventory.md (photo ID 2026-08-21)

PURPOSE:
  250-point first games using owned Blood Ravens Firstborn where possible,
  with legacy Primaris paths kept for when those bodies are completed.

PRIMARY_AUDIENCE:
  - First-time Space Marine player (son) vs Necrons learning games

UPDATE_TRIGGER:
  Inventory changes, MFM revision, or Chaplain / loadout proxy notes change.
-->

# Starter list - 250 points

A first game should be small, short, and mostly about learning the phase order. 250 points is roughly two or three units a side and finishes in an evening.

**Paint / rules:** Blood Ravens paint; **Codex Space Marines** + [Gladius Task Force](Gladius_Task_Force.md). Deathwatch models stay out of these lists.

**Every points value below is from Munitorum Field Manual (Marines) v1.2** (printed 13 Aug 2026).

---

## Owned path BR-1 — Terminators: 235 / 250

Best “few models, big armour” first game.

| Unit | Models | MFM Marines v1.2 | Ownership |
|------|--------|------------------|-----------|
| **Chaplain in Terminator Armour** (Leader) | 1 | **75** | **Owned** |
| **Terminator Squad** | 5 | **160** | **Owned** |
| | | **235** | |

Attach the **Chaplain to the Terminator Squad**.

**Model note:** the Terminator Chaplain’s power claw counts as a **Storm Shield** in game (owner).

Fifteen points spare — leave them.

---

## Owned path BR-2 — Captain + Tacticals: 220 / 250

More bodies, more Objective Control practice.

| Unit | Models | MFM Marines v1.2 | Ownership |
|------|--------|------------------|-----------|
| **Captain** (Leader) | 1 | **80** | **Owned** (Firstborn Captain) |
| **Tactical Squad** | 10 | **140** | **Owned** (`Tactical_sqaud 1` — includes **flamer**) |
| | | **220** | |

Attach the **Captain to the Tactical Squad**.

---

## Legacy Path A — Primaris (when squads are complete): 235 / 250

Use when Intercessor / Assault Intercessor bodies reach legal fives (BT set + AoD dual-use). Until then prefer BR-1 / BR-2.

| Unit | Models | MFM Marines v1.2 | Ownership |
|------|--------|------------------|-----------|
| **Captain** (Leader) | 1 | **80** | Owned (`AOD_Captain` when used for KT/40K dual-use). **Not** the Gravis relic-shield Captain — that model is **40K only** |
| **Intercessor Squad** | 5 | **80** | Partial — need five bolt-rifle bodies |
| **Assault Intercessor Squad** | 5 | **75** | Partial — need five melee bodies |
| | | **235** | |

---

## What this list is teaching you

| Unit | Job |
|------|-----|
| **Terminators + Chaplain** | Survive, walk up midboard, learn saves / Feel No Pain / Leader attach |
| **Captain + Tacticals** | Objective Control, four-step shooting, character in a squad |
| **Primaris path** | Same lessons on modern kits once squads are full |

**Every Command phase:**

1. Pick **Oath of Moment** — [`Oath_of_Moment.md`](Oath_of_Moment.md)
2. Decide whether to spend a **Combat Doctrine** — [`Gladius_Task_Force.md`](Gladius_Task_Force.md)

---

## Before the first game

- [ ] Confirm models against [`Owned_Models_Inventory.md`](Owned_Models_Inventory.md)
- [ ] Pick BR-1 or BR-2
- [ ] Write the three Combat Doctrines at the top of the list with tick boxes
- [ ] Re-check points against the current Marines MFM
- [ ] Print [`Quick_Reference_Play_Guide.md`](Quick_Reference_Play_Guide.md)

---

## Related pages

- [`Starter_500.md`](Starter_500.md) · [`Starter_750.md`](Starter_750.md) · [`Starter_1000.md`](Starter_1000.md)
- [`Gladius_Task_Force.md`](Gladius_Task_Force.md) · [`Oath_of_Moment.md`](Oath_of_Moment.md)
- [`Owned_Models_Inventory.md`](Owned_Models_Inventory.md)

---

## Change Log
- v0.6.1 (2026-08-21): Loadout notes — Tac flamer; Dev weapon list; Gravis Captain excluded from Primaris Captain slot.
- v0.6.0 (2026-08-21): Owned Blood Ravens paths BR-1 / BR-2; Chaplain claw = Storm Shield; Primaris path marked legacy/partial.
- v0.5.1 (2026-08-21): Ownership honesty after photo audit.
- v0.5.0 (2026-08-18): Project-wide semver snapshot (x.y.z).
- v1.0 (2026-08-16): Initial 250-point learning list (slice S5).

## Attribution
- Project: Wargame_Concierge · Maintainer: Russell Catt
- Warhammer 40,000 is a trademark of Games Workshop Limited. Teaching paraphrase only.

## Rising Tide Notes
- **Verify before you play.** Points reflect MFM Marines v1.2 (13 Aug 2026 print).
