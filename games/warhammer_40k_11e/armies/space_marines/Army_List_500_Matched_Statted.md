<!--
FILE: games/warhammer_40k_11e/armies/space_marines/Army_List_500_Matched_Statted.md
VERSION: v1.0 (2026-09-18)
OWNER: Russell Catt
AUTHOR_OF_NOTES: Cursor (cloud agent — son play pack)

DOCUMENT_TYPE: Army List / Fully Statted Play Sheet
PROJECT_NAME: Wargame_Concierge
GAME_SYSTEM: Warhammer 40,000 - 11th Edition
FACTION: Space Marines (Blood Ravens paint)
DETACHMENT: Gladius Task Force
REFERENCE_STATUS: Active — owned 500 Matched path for son. Profiles paraphrased from Wahapedia wh40k11ed (retrieved 2026-09-18); points MFM Marines v1.3 (owner paste 2026-08-27). Cross-check owned faction pack before events.

SOURCES:
  - https://wahapedia.ru/wh40k11ed/factions/space-marines/ (retrieved 2026-09-18)
  - games/warhammer_40k_11e/armies/space_marines/Starter_500_Matched.md
  - games/warhammer_40k_11e/armies/space_marines/Owned_Models_Inventory.md
  - games/warhammer_40k_11e/armies/space_marines/Oath_of_Moment.md

PURPOSE:
  Fully statted 500-point Gladius list for the son’s Blood Ravens learning force.

PRIMARY_AUDIENCE:
  - Son at the table with How_Your_Army_Works_500.md
  - Dad coaching

UPDATE_TRIGGER:
  Inventory loadout change, MFM revision, or faction pack errata to these datasheets.
-->

# Army list — 500 pts Matched — Gladius (statted)

> **FORCE DISPOSITION — write on the list before you deploy**  
> **Priority Assets** · MFM: `GLADIUS TASK FORCE - 3DP - PRIORITY ASSETS`  
> How matching works: [`../../setup/Chapter_Approved_Force_Dispositions.md`](../../setup/Chapter_Approved_Force_Dispositions.md) · Wound chart: [`../../rules/Wound_Roll_Reference.md`](../../rules/Wound_Roll_Reference.md) · How to play this list: [`How_Your_Army_Works_500.md`](How_Your_Army_Works_500.md) · System cheat sheet (wounds on-sheet): [`print/40k_11e_cheat_sheet_wounds.html`](print/40k_11e_cheat_sheet_wounds.html)

| Field | Value |
|-------|-------|
| **Detachment** | Gladius Task Force |
| **Force Disposition** | **Priority Assets** (`GLADIUS TASK FORCE - 3DP - PRIORITY ASSETS`) |
| **Points** | **500 / 500** |
| **Warlord** | Captain (Firstborn) |
| **Paint** | Blood Ravens |
| **Companion** | [`How_Your_Army_Works_500.md`](How_Your_Army_Works_500.md) |
| **confidence** | draft — Wahapedia 2026-09-18 + MFM v1.3 points; verify vs owned faction pack |

---

## Roster

| # | Unit | Models | Pts | Role |
|---|------|--------|-----|------|
| 1 | **Captain** *(Warlord)* | 1 | **80** | Leader → Tactical Squad |
| 2 | **Tactical Squad** | 10 | **140** | Midboard OC brick |
| 3 | **Devastator Squad** | 5 | **120** | Gunline |
| 4 | **Terminator Squad** | 5 | **160** | Armour / Deep Strike brick |
| | | | **500** | |

### Battle Formations (declare before deployment)

| Bodyguard | Leader |
|-----------|--------|
| Tactical Squad (10) | Captain |

Terminators and Devastators are separate units (no character on them at 500).

---

## Honesty / kitchen-table notes (read once)

| Model truth | Matched table call |
|-------------|--------------------|
| Captain: lightning claw + combi-weapon on the plastic | Count as **master-crafted power weapon** + **master-crafted boltgun** (current Captain datasheet options) unless opponent agrees otherwise |
| Devastators: meltagun on the fifth body | **Sergeant** — meltagun is **not** on the Devastator Sergeant list. Field as bolt pistol + Astartes chainsword (or boltgun). Four heavies stay legal |
| Terminators: assault cannon **and** cyclone on the same 5 | Only **one** storm-bolter upgrade per five models. **Pick one** for the list (recommend **assault cannon** for learning volume, or **cyclone** for krak shots). The other model shoots as storm bolter + power fist |
| Two SB bodies | **Chainfists** (owner photo 2026-09-18) — legal |

---

## 1 — Captain (Warlord) — 80 pts

| | |
|--|--|
| **Profile** | M6" · T4 · Sv3+ · W5 · Ld6+ · OC1 · **4+ invulnerable** |
| **Attaches to** | Tactical Squad (this list) |
| **Table loadout (count-as)** | Master-crafted boltgun; master-crafted power weapon; bolt pistol optional on datasheet defaults — use the count-as row below |
| **Role** | Midboard Leader; **Rites of Battle** (1CP discount once/round); **Finest Hour** once/battle in Fight |

| Weapon | Range | A | BS/WS | S | AP | D | Keywords |
|--------|-------|---|-------|---|----|---|----------|
| Master-crafted boltgun | 24" | 2 | 2+ | 4 | -1 | 2 | — |
| Master-crafted power weapon | Melee | 6 | 2+ | 5 | -2 | 2 | — |

**Abilities (paraphrase):** Leader; Oath of Moment; Rites of Battle; Finest Hour. Iron Halo → 4++.

---

## 2 — Tactical Squad — 140 pts

| Model | M | T | Sv | W | Ld | OC |
|-------|---|---|----|---|----|----|
| Tactical Marine / Sergeant | 6" | 4 | 3+ | 2 | 6+ | 2 |

**Owned loadout:** Sergeant bolt pistol + Astartes chainsword; **1 flamer**; **1 missile launcher**; 7× boltgun (+ pistols / close combat as datasheet).

| Weapon | Range | A | BS/WS | S | AP | D | Keywords |
|--------|-------|---|-------|---|----|---|----------|
| Bolt pistol | 12" | 1 | 3+ | 4 | 0 | 1 | [PISTOL] |
| Boltgun | 24" | 2 | 3+ | 4 | 0 | 1 | — |
| Flamer | 12" | D6 | N/A | 4 | 0 | 1 | [IGNORES COVER] [TORRENT] |
| Missile launcher — frag | 48" | D6 | 4+ | 4 | 0 | 1 | [BLAST] [HEAVY] |
| Missile launcher — krak | 48" | 1 | 4+ | 9 | -2 | D6 | [HEAVY] |
| Astartes chainsword | Melee | 4 | 3+ | 4 | -1 | 1 | — |
| Close combat weapon | Melee | 2 | 3+ | 4 | 0 | 1 | — |

**Abilities:** Oath of Moment. With Captain attached → one unit for rules / coherency (**2″/9″**, `03.03`).

---

## 3 — Devastator Squad — 120 pts

| Model | M | T | Sv | W | Ld | OC |
|-------|---|---|----|---|----|----|
| Devastator Marine / Sergeant | 6" | 4 | 3+ | 2 | 6+ | 1 |

**Owned heavy four:** heavy bolter · multi-melta · plasma cannon · lascannon. **Fifth body:** Sergeant (see honesty table — not a fifth heavy).

| Weapon | Range | A | BS | S | AP | D | Keywords |
|--------|-------|---|----|---|----|---|----------|
| Heavy bolter | 36" | 3 | 4+ | 5 | -1 | 2 | [HEAVY] [SUSTAINED HITS 1] |
| Multi-melta | 18" | 2 | 4+ | 9 | -4 | D6 | [HEAVY] [MELTA 2] |
| Plasma cannon — standard | 36" | D3 | 4+ | 7 | -2 | 1 | [BLAST] [HEAVY] |
| Plasma cannon — supercharge | 36" | D3 | 4+ | 8 | -3 | 2 | [BLAST] [HEAVY] [HAZARDOUS] |
| Lascannon | 48" | 1 | 4+ | 12 | -3 | D6+1 | [HEAVY] |
| Bolt pistol (Sergeant) | 12" | 1 | 3+ | 4 | 0 | 1 | [PISTOL] |
| Astartes chainsword (Sergeant) | Melee | 4 | 3+ | 4 | -1 | 1 | — |

**Abilities (paraphrase):** Oath of Moment; **Signum** — if the unit Remained Stationary, ranged weapons get **[IGNORES COVER]** until your next Movement phase; **Armorium Cherub** once/battle (turn one Hit into unmodified 6).

**Habit:** Remain Stationary when you can — Signum is the unit’s best trick.

---

## 4 — Terminator Squad — 160 pts

| Model | M | T | Sv | W | Ld | OC |
|-------|---|---|----|---|----|----|
| Terminator | 5" | 5 | 2+ | 3 | 6+ | 1 |

**4+ invulnerable** on the datasheet.

**Matched loadout for this pack (pick one heavy):**

| Model | Ranged | Melee |
|-------|--------|-------|
| Sergeant | Storm bolter | Power weapon |
| Heavy (choose) | **Assault cannon** *or* **Cyclone** (+ storm bolter on cyclone body) | Power fist |
| 2× chainfist brothers | Storm bolter | **Chainfist** |
| Remaining | Storm bolter | Power fist |

| Weapon | Range | A | BS/WS | S | AP | D | Keywords |
|--------|-------|---|-------|---|----|---|----------|
| Storm bolter | 24" | 2 | 3+ | 4 | 0 | 1 | [RAPID FIRE 2] |
| Assault cannon | 24" | 6 | 3+ | 6 | 0 | 1 | [DEVASTATING WOUNDS] |
| Cyclone — frag | 36" | 2D6 | 3+ | 4 | 0 | 1 | [BLAST] |
| Cyclone — krak | 36" | 2 | 3+ | 9 | -2 | D6 | — |
| Power weapon | Melee | 4 | 3+ | 5 | -2 | 1 | — |
| Power fist | Melee | 3 | 3+ | 8 | -2 | 2 | — |
| Chainfist | Melee | 3 | 4+ | 8 | -2 | 2 | [ANTI-VEHICLE 3+] |

**Abilities (paraphrase):** Oath of Moment; **Deep Strike**; **Fury of the First** (+1 to Hit vs your Oath target); Teleport Homer token (optional setup — confirm on pack).

**Reserves kit (son summary):** see [`How_Your_Army_Works_500.md`](How_Your_Army_Works_500.md) — Strategic Reserves / Deep Strike / Rapid Ingress.

---

## Gladius reminders (one line each)

| Tool | When |
|------|------|
| **Oath of Moment** | Your Command phase — name one enemy unit; re-roll Hits; +1 Wound if Codex SM purity |
| **Combat Doctrines** | Optional; each once/battle (Devastator / Tactical / Assault) |
| **Adaptive Strategy** | 1CP — one unit into any doctrine |

---

## Related

- [`Starter_500_Matched.md`](Starter_500_Matched.md) — roster origin  
- [`Oath_of_Moment.md`](Oath_of_Moment.md) · [`Gladius_Task_Force.md`](Gladius_Task_Force.md)  
- [`Quick_Reference_Play_Guide.md`](Quick_Reference_Play_Guide.md)  
- Print: [`print/40k_sm_army_list_500.html`](print/40k_sm_army_list_500.html)

---

## Games Workshop notice

**UNOFFICIAL.** This document is completely unofficial and in no way endorsed by Games Workshop Limited. Personal / no-charge use only — never for sale. Warhammer, Warhammer 40,000 and associated marks are trademarks of Games Workshop Limited. Used without permission. No challenge to their status intended. Warhammer 40,000 is Copyright Games Workshop Limited. Teaching notes by Russell Catt (Wargame Concierge). Games Workshop retains IP in the settings and characters. No official logos.

## Change Log

- v1.0 (2026-09-18): Initial statted 500 Matched pack for son (Captain + Tac + Dev + Terms); loadout honesty notes; Wahapedia 2026-09-18 profiles.

## Attribution

- Project: Wargame_Concierge · Maintainer: Russell Catt
